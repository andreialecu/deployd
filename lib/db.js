...
function getConnection(db) {
  ...
  return new Promise(function(resolve, reject) {
    try {
      mongodb.MongoClient.connect(db.connectionString, db.connectionOptions, function (err, client) {
...
Store.prototype.count = function(query, fn) {
  ...
  collection(this, function (err, col) {
    ...
    col.find(query, {...options, projection: fields }).count(function(err, count) {
      if (err) return fn(err);
      fn(null, count);
    });
  });
...
Store.prototype.find = function (query, fn) {
  ...
  collection(this, function (err, col) {
    ...
    if(typeof query._id === 'string') {
      ...
      if(fields) {
        col.find(query, { ...options, projection: fields }).toArray(function (err, arr) {
          if (err) {
            fn(err);
            return;
          }
